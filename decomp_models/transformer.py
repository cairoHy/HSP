...
class Transformer(Model):
    ...
    def _embed(self):
        with tf.device('/cpu:0'), tf.variable_scope('word_embedding'):
            self.word_embeddings = get_embedding_may_pretrain(self.vocab_size, self.embed_dim, self.params, 'word_emb')
    ...
    def _encode(self):
        ...
        bias = tf.get_variable("src_language_bias", [self.hidden_size])
        ...
        self.encoder_output = transformer_encoder(self.encoder_input, self.enc_attn_bias, self.params)
    ...
    def _decode_train(self):
        ...
        self.all_att_weights, self.decoder_output = transformer_decoder(self.decoder_input, self.encoder_output,
                                                                        ...
                                                                        self.params)
        # [b, q_2, e] => [b*q_2, v]
        self.decoder_output = tf.reshape(self.decoder_output, [-1, self.hidden_size])
        self.logits = tf.matmul(self.decoder_output, self.decoder_weights, False, True)
        self._compute_loss()
    ...
    def _compute_loss(self):
        ...
        self.ce = tf.reshape(self.ce, tf.shape(self.tgt_seq))
    ...
    def decode_infer(self, inputs, state):
        ...
        self.all_att_weights, self.decoder_output, self.decoder_state = transformer_decoder(self.infer_decoder_input,
                                                                                            state['encoder'],
                                                                                            self.infer_dec_attn_bias,
                                                                                            self.enc_attn_bias,
                                                                                            self.params,
                                                                                            state=state['decoder'])
        self.decoder_output = self.decoder_output[:, -1, :]  # [batch_size, hidden]
        self.logits = tf.matmul(self.decoder_output, self.decoder_weights, False, True)  # [batch_size, vocab_size]
    ...
    def get_parameters():
        params = tf.contrib.training.HParams(
            ...
            use_copy=False,  # use copy mechanism in decoder
            ...
            use_pos=False,  # use POS annotation
            ner_emb_size=30,  # the dimension of ner feature
            use_pretrained_embedding=False,  # use pretrained word embedding
            embed_dim=512,  # pretrained word embedding dimension
        )
