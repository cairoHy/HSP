...
from decomp_models.model_utils.module import transformer_encoder, transformer_decoder, transformer_concated_decoder
from utils.optimize import smooth_cross_entropy
from utils.train_utils import tf_trunct


# noinspection PyAttributeOutsideInit
class Seq2SeqSketchV7(Model):
    """
    Transformer + CopyV2 + pos_annotation + sketch decoding
    separate copy prob calculation
    adjust for universal embedding
    ######################### shape notation def
    # b ---> batch_size
    # q ---> word(token) number
        .........q_1 for source
    ...
    def __init__(self, params, scope, mode='train', initializer=None):
        self.sketch_loss_alpha = 0.1
        self.initializer = initializer
        self.mode = mode
        super(Seq2SeqSketchV7, self).__init__(params=params, scope=scope)
    ...
    def _setup_placeholder(self):
        ...
        # sketch
        self.sketch_seq = tf.placeholder(tf.int32, [None, None], name='sketch_seq')  # [b, q_3]
        self.sketch_len = tf.placeholder(tf.int32, [None], name='sketch_len')  # [b]
    ...
    def _sketch_decode_train(self):
        state = {'encoder': self.encoder_output}
        self.sketch_logits = self._decode_func(
            self.sketch_seq, self.sketch_len, self.sketch_embeddings, self.sketch_decoder_weights,
            self.comp_q_attn_bias, 'train', state, self.sketch_vocab_size, use_copy=False,
            scope='sketch_decoder', decode_sketch=True)

    def _sketch_encode(self):
        sketch_mask = tf.sequence_mask(self.sketch_len,
                                       maxlen=tf.shape(self.sketch_seq)[1],
                                       dtype=tf.float32)  # [b, q_3]
        pos_anno = tf.zeros_like(sketch_mask, dtype=tf.int32)
        self.sketch_encoder_output, _ = self._encode_func(self.sketch_seq, self.sketch_len,
                                                          self.sketch_embeddings,
                                                          scope='sketch_encoder',
                                                          use_pos_anno=True, pos_anno=pos_anno)
        self.concated_encoder_output = self.concat_padded_seq(self.encoder_output, self.sketch_encoder_output)
    ...
    def prepare_for_final_decode(self):
        ...
        sketch_mask = tf.sequence_mask(self.sketch_len,
                                       maxlen=tf.shape(self.sketch_seq)[1],
                                       dtype=tf.float32)
        self.concat_src_mask = tf.concat([src_mask, sketch_mask], -1)  # [b, q_1 + q_3]
        self.concat_src_true_mask = tf.sequence_mask(self.src_len,
                                                     maxlen=tf.shape(self.concated_encoder_output)[1],
                                                     dtype=tf.float32)  # [b, q_1 + q_3]
        self.final_enc_attn_bias = attention_bias(self.concat_src_mask, "masking")
        self.expand_source_ids_oo = tf.concat([self.source_ids_oo, self.sketch_seq], axis=-1)  # [b, q_1 + q_3]
    ...
    def _decode_train(self):
        ...
        def transformer_concated_decoder_internal(inputs, memory, bias, mem_bias, params, state=None, scope=None,
                                                  reuse=False):
            return transformer_concated_decoder(inputs, memory, bias, mem_bias, params, state, scope,
                                                reuse, src_true_mask=self.concat_src_true_mask)

        self.final_decode_logits = self._decode_func(
            ...
            expand_source_ids_oo=self.expand_source_ids_oo,
            max_out_oovs=self.max_out_oovs, src_mask=self.concat_src_mask,
            decoder_fn=transformer_concated_decoder_internal,
            scope='final_decoder')

    def _compute_loss(self):
        # label smoothing, sketch cross-entropy
        self.sketch_ce = smooth_cross_entropy(
            self.sketch_logits,
            self.sketch_seq,
            self.params.label_smoothing)
        self.sketch_ce = tf.reshape(self.sketch_ce, tf.shape(self.sketch_seq))  # [batch, q_3]
        sketch_mask = tf.sequence_mask(self.sketch_len,
                                       maxlen=tf.shape(self.sketch_seq)[1],
                                       dtype=tf.float32)
        self.sketch_loss = tf.reduce_sum(self.sketch_ce * sketch_mask) / tf.reduce_sum(sketch_mask)

        # label smoothing, sub-query cross-entropy
        self.ce = smooth_cross_entropy(
            self.final_decode_logits,
            self.tgt_label,
            self.params.label_smoothing)
        ...
        self.loss = tf.reduce_sum(self.ce * tgt_mask) / tf.reduce_sum(tgt_mask)

        self.loss += self.sketch_loss_alpha * self.sketch_loss
    ...
    def decode_infer_stage_1(self, inputs, state):
        ...
        return self._decode_func(target_sequence, target_length, self.sketch_embeddings, self.sketch_decoder_weights,
                                 self.comp_q_attn_bias, 'infer', state, self.sketch_vocab_size, use_copy=False,
                                 scope='sketch_decoder', decode_sketch=True)
    ...
    def calculate_final_logits(decoder_output, all_att_weights, vocab_probs, source_ids_oo, max_out_oovs, src_mask,
        ...
        avg_att_weights = all_att_weights  # [b, q_2, q_1 + q_3]
    ...
    def make_feed_dict(self, batch):
        ...
        feed_dict = {self.src_seq: batch.source_ids,
                     ...
                     self.sketch_seq: batch.sketch_ids,
                     self.sketch_len: batch.sketch_len}
        return feed_dict
