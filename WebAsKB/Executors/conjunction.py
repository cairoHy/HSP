...
class Conjunction(ExecutorBase):
    ...
    def gen_webanswer_first_wave(self,split_points):
        ex_split_points = split_points[split_points[self.comp_type] == 'conjunction']
        y1 = ex_split_points[[self.first_q_name,'answers', self.id_name]]
        y1.columns = ['question','answers', self.id_name]
        y1 = y1.to_dict(orient='rows')
        y2 = ex_split_points[[self.second_q_name,'answers', self.id_name]]
        y2.columns = ['question','answers', self.id_name]
    ...
    def proc_webanswer_wave_results(self,split_points,webanswer_dict):
        ...
        for ind,question in ex_split_points.iterrows():
            if question[self.first_q_name]!=question[self.first_q_name] \
                    or question[self.second_q_name]!=question[self.second_q_name]:
                continue
            ...
            try:
                ...
                answers1 = ''
            try:
                answers2 = pd.DataFrame(webanswer_dict[question[self.second_q_name] + question[self.id_name]])
            except KeyError:
                answers2 = ''
