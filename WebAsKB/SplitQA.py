...
class SplitQA():
    def __init__(self):
        ...
        # load my split point and merge with split_points dataframe
        data = pd.read_csv(config.my_decomp_res_dir + 'split_res_{}_{}.csv'.format(config.EVALUATION_SET,
                                                                                   config.split_model))
        data.rename(columns={'id': 'ID', 'pred_query_first': 'first_query',
                             'pred_query_second': 'second_query', 'question': 'complexQ'}, inplace=True)
        data = data[['ID', 'complexQ', 'first_query', 'second_query', 'bleu-4', 'rouge-l', 'status',
                     'pred_query_type']]
        data = data.dropna()
        self.split_points = self.split_points.merge(data, on='ID', how='inner')
    ...
    def run_executors(self):
        ...
        # Load my RC answers
            ...
            with open(config.my_rc_answer_dir + 'rc_ans_{}.json'.format(config.EVALUATION_SET)) as fin:
                webanswer_dict = json.load(fin)
            for key in webanswer_dict.keys():  # turn to same format
                webanswer_dict[key] = pd.DataFrame(webanswer_dict[key])

        for question in question_list:
            ques_key = question['question'] + question['ID'] if config.use_my_decomp else question['question']
            if ques_key not in webanswer_dict:
                print('Warning!!! RC cache is missing the following question: ' + ques_key)
        ##################################
        # run executors
